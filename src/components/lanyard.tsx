...
// rapier bodies get a scratch vector cached on them between frames
...
function Band({
    ...
    // Pin the strap to the top-right of the canvas. Rapier reads world
    // transforms once at mount, so this only re-anchors on remount, not resize.
    const { viewport } = useThree();
    const anchor: [number, number, number] = [
        viewport.width / 2 - 1.8,
    ...
    useFrame((state, delta) => {
        ...
        if (fixed.current && j1.current && j2.current && j3.current && card.current && band.current) {
            ...
            // rotation() is a quaternion; only its y term is used to damp the spin
            const rotY = card.current.rotation().y;
            card.current.setAngvel(
                { x: ang.x, y: ang.y - rotY * 0.25, z: ang.z },
                true,
            );
        }
    });
